...
export default function LargeFormatDarContent() {
  return (
    <>
      <section className="relative pt-32 pb-12 md:pb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
          <nav className="text-xs text-silver-500 mb-5 flex items-center gap-2">
            ...
            <span>/</span>
            <Link href="/services/large-format-printing" className="hover:text-promo-400">Large Format</Link>
            <span>/</span>
            <span className="text-promo-400 font-semibold">Dar es Salaam</span>
          </nav>

          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4">Large Format Printing in Dar es Salaam</h1>
          <p className="text-silver-300 text-lg max-w-3xl mb-6">Banners, roll-ups, backdrops for events and advertising. Installation included.</p>

          <div className="flex flex-wrap gap-3">
            <button onClick={() => trackEvent("quote_click", { label: "large_format_dar" })} className="px-7 py-3.5 rounded-full bg-gradient-to-r from-promo-500 to-promo-400 text-white font-semibold">Get Quote</button>
            <a href="https://wa.me/+255716002790" target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("whatsapp_click", { label: "large_format_dar" })} className="px-7 py-3.5 rounded-full bg-green-500/20 border border-green-500/30 text-green-300 font-semibold">WhatsApp</a>
            <a href="tel:+255716002790" onClick={() => trackEvent("phone_click", { label: "large_format_dar" })} className="px-7 py-3.5 rounded-full bg-white/10 border border-white/20 text-white font-semibold">Call 0716 002 790</a>
          </div>
        </div>
      </section>
