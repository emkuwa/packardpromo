...
export default function ProductDetail({ product }: { service: Service; product: Product & { category: string } }) {
  ...
  return <div className="detail-screen">
    <header className="detail-header"><Link href="/products" aria-label="Back to products">‹</Link><div><button aria-label="Save product">♡</button><button aria-label="Share product">⇧</button></div></header>
    <div className="detail-image"><Image src={product.image} alt={product.name} fill priority sizes="(max-width: 700px) 100vw, 700px"/><div className="image-dots"><i/><i className="active"/><i/><i/></div></div>
    <section className="detail-body">
      ...
      <div className="feature-grid"><span>▣ <small>High-quality print</small></span><span>▧ <small>Multiple options</small></span><span>▤ <small>Single or double sided</small></span><span>◷ <small>Fast turnaround</small></span></div>
      ...
      <Link href={`/ai-tools/quote-generator?product=${product.slug}&qty=${qty}`} className="add-quote">▤ Add to Quote</Link>
    </section>
  </div>;
}
