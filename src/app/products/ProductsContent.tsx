...
export default function ProductsContent({ services, initialSearch = "" }: { services: Service[]; initialSearch?: string }) {
  ...
  return <div className="products-screen">
    ...
    <div className="products-heading"><div><h1>All Products</h1><p>{filtered.length}+ products</p></div><button className="filter-button"><AppIcon name="filter" className="size-5"/> <span>Filter</span></button></div>
    {filtered.length ? <div className="product-grid">{filtered.map((product) => <article className="product-tile" key={product.id}>
      <div className="product-image"><Link className="absolute inset-0 block" href={`/products/${product.slug}`} aria-label={`View ${product.name}`}><Image src={appProductImage(product, product.image)} alt={product.name} fill sizes="(max-width: 700px) 50vw, 260px"/></Link><button type="button" aria-label={`Save ${product.name}`}><AppIcon name="heart"/></button></div>
